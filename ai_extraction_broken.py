...
def generate_detailed_reasoning(
    ...
    if validation_status == "invalid" and (extracted_value is None or str(extracted_value).strip() == ""):
        ...
        base_analysis = f"EXTRACTION ANALYSIS:\n❌ Field '{display_name}' could not be located in the document{context_text}\n\n"
        base_analysis += f"CONFIDENCE CALCULATION:\n• Base extraction confidence: 0% (field not found)\n"
        base_analysis += f"• Field-specific adjustments: None applied\n• Final confidence: {confidence_score}%\n\n"
        base_analysis += "RULES COMPLIANCE:\n• Missing required field data\n"
        base_analysis += "• No conflicting information found in knowledge base\n"
        base_analysis += "• Field appears to be absent from the provided document"

        suggested_action = f"SUGGESTED RESOLUTION:\nPlease verify the following with the document provider:\n\n"
        suggested_action += f"1. Is the '{display_name}' information included elsewhere in the document?\n"
        suggested_action += "2. Should this field be marked as optional or not applicable?\n"
        suggested_action += "3. Can you provide additional documentation that contains this information?\n"
        suggested_action += "4. Is there alternative terminology used for this field in your organization?\n\n"
        suggested_action += "RECOMMENDED QUESTIONS TO ASK:\n"
        suggested_action += f"• \"Could you confirm if {display_name.lower()} information is available for this document?\"\n"
        suggested_action += "• \"Is this field typically included in documents of this type?\"\n"
        suggested_action += "• \"Are there any supplementary documents that might contain this information?\""

    elif validation_status == "invalid":
        base_analysis = f"EXTRACTION ANALYSIS:\nWarning: Field '{display_name}' extracted but failed validation\n"
        base_analysis += f"• Extracted value: '{extracted_value}'\n• Expected type: {field_type}\n"
        base_analysis += "• Issue: Value format does not meet field type requirements\n\n"
        base_analysis += f"CONFIDENCE CALCULATION:\n• Base extraction confidence: N/A%\n"
        base_analysis += f"• Format validation: Failed\n• Final confidence: {confidence_score}%\n\n"
        base_analysis += f"RULES COMPLIANCE:\n• Value found but incorrect format\n• Does not meet {field_type} field requirements"

        if field_type == "DATE":
            suggested_action = f"SUGGESTED RESOLUTION:\nThe extracted value '{extracted_value}' is not in the expected date format (YYYY-MM-DD).\n\n"
            suggested_action += "RECOMMENDED QUESTIONS TO ASK:\n"
            suggested_action += f"• \"Can you confirm the correct date for {display_name.lower()}?\"\n"
            suggested_action += "• \"Is the date format in the document non-standard?\"\n"
            suggested_action += "• \"Should this be interpreted as a different type of date field?\"\n\n"
            ...
            suggested_action += "RECOMMENDED QUESTIONS TO ASK:\n"
            suggested_action += f"• \"What is the correct numeric value for {display_name.lower()}?\"\n"
            suggested_action += "• \"Is this field meant to contain text instead of numbers?\"\n"
            suggested_action += "• \"Are there special formatting rules for this numeric field?\"\n\n"
            ...
            suggested_action += "RECOMMENDED QUESTIONS TO ASK:\n"
            suggested_action += f"• \"Can you confirm the correct value for {display_name.lower()}?\"\n"
            suggested_action += "• \"Is there additional context needed to interpret this field?\"\n"
            suggested_action += "• \"Should this field have a different data type or format?\"\n\n"
...
- When extracting data, check if values conflict with the knowledge documents above.
- If a field value conflicts with policies/requirements in knowledge documents, reduce confidence to 50% and note the conflict in your reasoning.
- For example: If a contract involves U.S. jurisdiction but knowledge documents require legal review for U.S. contracts, flag this as a potential conflict.
- Include references to specific knowledge document sections when conflicts are detected.
...
def process_extraction_session(session_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    print("=== PYTHON SCRIPT STARTING (WITH UPDATED SAMPLE DATA DETECTION) ===", file=sys.stderr)
    logging.basicConfig(level=logging.INFO, stream=sys.stderr)
    logging.info("=== PYTHON SCRIPT IS BEING EXECUTED ===")
    logging.info("=== SAMPLE DATA DETECTION SYSTEM IS ACTIVE ===")
...
# Test code removed to clean up the module