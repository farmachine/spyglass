...
export default function SchemaView() {
  ...
  // Debug mode state for manual steps
  const [currentStep, setCurrentStep] = useState(1);
  const [stepStatus, setStepStatus] = useState({
    step1: false, // Document extraction
    step2: false, // AI prompt generation
    step3: false, // AI extraction
    step4: false  // Save validations
  });
  const [stepData, setStepData] = useState<{
    extractedText: any | null;
    generatedPrompt: string | null;
    aiResponse: any | null;
    savedValidations: any[] | null;
  }>({
    extractedText: null,
    generatedPrompt: null,
    aiResponse: null,
    savedValidations: null
  });
...
=== END ERROR ===`);
  ...
  // Manual debug step handlers
  const handleStep1DocumentExtraction = async () => {
    ...
    try {
      ...
      if (response.message) {
        ...
        if (sessionData.extractedData) {
          const extractedData = JSON.parse(sessionData.extractedData);
          setStepData(prev => ({ ...prev, extractedText: extractedData }));
          
          const text = extractedData.extracted_texts?.map((doc: any, index: number) => 
            `--- DOCUMENT ${index + 1}: ${doc.file_name} (${doc.word_count} words) ---\n${doc.text_content}`
          ).join('\n\n--- DOCUMENT SEPARATOR ---\n\n') || '';
          
          setDocumentContent({ text, count: extractedData.extracted_texts?.length || 0 });
        }
      }
    } catch (error) {
      console.error('Step 1 error:', error);
      setError(`Step 1 failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoadingDocuments(false);
    }
  };

  const handleStep2PromptGeneration = async () => {
    if (!schemaData || !documentContent) return;
    
    try {
      const prompt = generateSchemaMarkdown(schemaData, documentContent.text, documentContent.count);
      setStepData(prev => ({ ...prev, generatedPrompt: prompt }));
      setStepStatus(prev => ({ ...prev, step2: true }));
      setCurrentStep(3);
    } catch (error) {
      console.error('Step 2 error:', error);
      setError(`Step 2 failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };

  const handleStep3AIExtraction = async () => {
    if (!stepData.generatedPrompt || !session) return;
    ...
    try {
      const response = await apiRequest(`/api/sessions/${sessionId}/gemini-extraction`, {
        method: 'POST',
        body: JSON.stringify({
          prompt: stepData.generatedPrompt,
          projectId: session.projectId
        }),
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (response.success) {
        setStepData(prev => ({ ...prev, aiResponse: response.extractedData }));
        setGeminiResponse(JSON.stringify(response.extractedData, null, 2));
        setStepStatus(prev => ({ ...prev, step3: true }));
        setCurrentStep(4);
      } else {
        setError(`AI extraction failed: ${response.error}`);
      }
    } catch (error) {
      console.error('Step 3 error:', error);
      setError(`Step 3 failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsProcessing(false);
    }
  };

  const handleStep4SaveValidations = async () => {
    ...
    try {
      const response = await apiRequest(`/api/sessions/${sessionId}/save-validations`, {
        method: 'POST',
        body: JSON.stringify({
          validations: Array.isArray(stepData.aiResponse) ? stepData.aiResponse : [stepData.aiResponse]
        }),
        headers: { 'Content-Type': 'application/json' }
      });
      
      if (response.success) {
        setStepData(prev => ({ ...prev, savedValidations: response.validations }));
        setSavedValidations(response.validations);
        setStepStatus(prev => ({ ...prev, step4: true }));
      } else {
        setError(`Save validations failed: ${response.error}`);
      }
    } catch (error) {
      console.error('Step 4 error:', error);
      setError(`Step 4 failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsSavingToDatabase(false);
    }
  };
    ...
    <div style={{ 
      ...
      {extractionMode === 'debug' && (
        <div style={{ 
          ...
          === DEBUG MODE: MANUAL EXTRACTION STEPS ===
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button
                onClick={handleStep1DocumentExtraction}
                disabled={isLoadingDocuments || currentStep !== 1}
                style={{
                  padding: '10px 15px',
                  backgroundColor: stepStatus.step1 ? '#28a745' : (currentStep === 1 ? '#007bff' : '#6c757d'),
                  color: 'white',
                  border: 'none',
                  borderRadius: '5px',
                  cursor: currentStep === 1 && !isLoadingDocuments ? 'pointer' : 'not-allowed',
                  fontWeight: 'bold',
                  opacity: currentStep === 1 && !isLoadingDocuments ? 1 : 0.6
                }}
              >
                {isLoadingDocuments ? 'Processing...' : 'Step 1: Extract Text'}
                {stepStatus.step1 && ' ✓'}
              </button>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button
                onClick={handleStep2PromptGeneration}
                disabled={currentStep !== 2}
                style={{
                  padding: '10px 15px',
                  backgroundColor: stepStatus.step2 ? '#28a745' : (currentStep === 2 ? '#007bff' : '#6c757d'),
                  color: 'white',
                  border: 'none',
                  borderRadius: '5px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button
                onClick={handleStep3AIExtraction}
                disabled={isProcessing || currentStep !== 3}
                style={{
                  padding: '10px 15px',
                  backgroundColor: stepStatus.step3 ? '#28a745' : (currentStep === 3 ? '#007bff' : '#6c757d'),
                  color: 'white',
                  border: 'none',
                  borderRadius: '5px',
                  cursor: currentStep === 3 && !isProcessing ? 'pointer' : 'not-allowed',
                  fontWeight: 'bold',
                  opacity: currentStep === 3 && !isProcessing ? 1 : 0.6
                }}
              >
                {isProcessing ? 'AI Processing...' : 'Step 3: AI Extract'}
                {stepStatus.step3 && ' ✓'}
              </button>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button
                onClick={handleStep4SaveValidations}
                disabled={isSavingToDatabase || currentStep !== 4}
                style={{
                  padding: '10px 15px',
                  backgroundColor: stepStatus.step4 ? '#28a745' : (currentStep === 4 ? '#007bff' : '#6c757d'),
                  color: 'white',
                  border: 'none',
                  borderRadius: '5px',
                  cursor: currentStep === 4 && !isSavingToDatabase ? 'pointer' : 'not-allowed',
                  fontWeight: 'bold',
                  opacity: currentStep === 4 && !isSavingToDatabase ? 1 : 0.6
                }}
              >
                {isSavingToDatabase ? 'Saving...' : 'Step 4: Save Data'}
                {stepStatus.step4 && ' ✓'}
              </button>
            </div>
          </div>

          {/* Current Step Data Display */}
          {extractionMode === 'debug' && (
            <div style={{ marginTop: '20px' }}>
              {stepData.extractedText && (
                <div style={{ 
                  marginBottom: '15px',
                  padding: '10px',
                  ...
                  <strong>Step 1 Complete:</strong> Extracted {stepData.extractedText.extracted_texts?.length || 0} documents
                </div>
              )}
              
              {stepData.generatedPrompt && (
                <div style={{ 
                  marginBottom: '15px',
                  padding: '10px',
                  backgroundColor: '#d4edda',
                  ...
                  <strong>Step 2 Complete:</strong> Generated AI prompt ({stepData.generatedPrompt.length} characters)
                </div>
              )}
              ...
              {stepData.aiResponse && (
                <div style={{ 
                  ...
                  <strong>Step 3 Complete:</strong> AI extraction completed
                </div>
              )}
              
              {stepData.savedValidations && (
                <div style={{ 
                  marginBottom: '15px',
                  padding: '10px',
                  backgroundColor: '#d4edda',
                  ...
                  <strong>Step 4 Complete:</strong> Saved {stepData.savedValidations.length} validations to database
