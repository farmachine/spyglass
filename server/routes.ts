...
`]);
  ...
  // Automated processing endpoint - combines text extraction + AI extraction + database save
  app.post("/api/sessions/:sessionId/process", async (req, res) => {
    try {
      const sessionId = req.params.sessionId;
      const { files, project_data } = req.body;
      
      console.log(`AUTOMATED PROCESSING: Starting for session ${sessionId}`);
      console.log(`Processing ${files?.length || 0} documents with project ${project_data?.projectId}`);
      ...
      // Call the AI extraction script directly with all data
      const { spawn } = await import('child_process');
      let output = '';
      let error = '';
      ...
      // Send complete data to Python script for processing
      const pythonInput = JSON.stringify({
        projectId: project_data.projectId,
        sessionId: sessionId,
        documents: convertedFiles,
        schemaFields: project_data.schemaFields || [],
        collections: project_data.collections || []
      });
      ...
      await new Promise((resolve, reject) => {
        python.on('close', async (code: any) => {
          if (code !== 0) {
            ...
            return reject(new Error(`Automated processing failed: ${error}`));
          }
          
          try {
            const result = JSON.parse(output);
            console.log(`AUTOMATED PROCESSING: Successfully processed ${files?.length || 0} documents`);
            ...
            console.error('AUTOMATED PROCESSING JSON parse error:', parseError);
            console.error('Raw output:', output);
            reject(new Error(`Invalid JSON response: ${parseError}`));
          }
        });
      ...
      res.status(500).json({ 
        success: false,
        error: error.message,
        message: "Automated processing failed" 
      });
    }
  });
