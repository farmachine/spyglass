...
`]);
  ...
  // ASYNC PROCESS: Start extraction process asynchronously 
  app.post("/api/sessions/:sessionId/process", async (req, res) => {
    try {
      ...
      console.log(`ASYNC PROCESS: Starting async extract+validate for session ${sessionId}`);
      ...
      processExtractionAsync(sessionId, files, project_data, storage);
  ...
  // SYNC PROCESS: Extract and validate in one AI call (for testing/backup)
  app.post("/api/sessions/:sessionId/process-sync", async (req, res) => {
    try {
      const sessionId = req.params.sessionId;
      const { files, project_data } = req.body;
      const projectId = project_data?.projectId || project_data?.id;
...
`]);
  ...
  // Session status polling endpoint for async processing
  app.get("/api/sessions/:sessionId/status", async (req, res) => {
    try {
      ...
      res.json({
        ...
        progressMessage: "Processing...", // Use static message for now
        errorMessage: null, // Skip error message field for now
...
// ASYNC PROCESSING FUNCTION: Handles background extraction processing
async function processExtractionAsync(sessionId: string, files: any[], project_data: any, storage: any) {
  try {
    const projectId = project_data?.projectId || project_data?.id;
    
    console.log(`ASYNC PROCESSING: Starting background processing for session ${sessionId}`);
    
    // Update progress - skip for now due to schema limitations
    console.log("ASYNC PROCESSING: Converting documents...");
    ...
    console.log(`ASYNC PROCESSING: Processing ${convertedFiles.length} documents with integrated extraction and validation`);
    
    // Update progress - skip for now due to schema limitations
    console.log("ASYNC PROCESSING: Loading project configuration...");
    ...
    // Update progress - skip for now due to schema limitations
    console.log("ASYNC PROCESSING: Preparing AI extraction...");
    ...
    // Update progress - skip for now due to schema limitations
    console.log("ASYNC PROCESSING: Running AI extraction...");
    ...
    await new Promise((resolve, reject) => {
      python.on('close', async (code: any) => {
        if (code !== 0) {
          ...
          console.error("ASYNC PROCESSING: AI extraction failed:", error);
        ...
        try {
          // Update progress - skip for now due to schema limitations
          console.log("ASYNC PROCESSING: Processing AI results...");

          const result = JSON.parse(output);
          console.log('ASYNC PROCESSING result:', JSON.stringify(result, null, 2));
          ...
          if (!result.success) {
            ...
            console.error("ASYNC PROCESSING: Extraction failed:", result.error || 'Single-step extraction failed');
            return reject(new Error(result.error || 'Single-step extraction failed'));
          }
          ...
          // Update progress - skip for now due to schema limitations
          console.log("ASYNC PROCESSING: Saving batch information...");
          ...
          if (batchData) {
            ...
            await storage.createSessionBatch({
              ...
              extractionPrompt: batchData.extractionPrompt,
              aiResponse: batchData.aiResponse,
              inputTokenCount: batchData.inputTokenCount,
              outputTokenCount: batchData.outputTokenCount,
              validationCount: batchData.validationCount || 0
            });
          }

          // Update progress - skip for now due to schema limitations
          console.log("ASYNC PROCESSING: Saving field validations...");
          ...
          for (const validation of fieldValidations || []) {
            try {
              const savedValidation = await storage.createFieldValidation({
                ...
                extractedValue: validation.extracted_value,
                originalExtractedValue: validation.original_extracted_value,
                originalConfidenceScore: validation.original_confidence_score || 0,
                ...
                documentSections: validation.document_sections,
                batchNumber: validation.batch_number || 1
              });
              savedValidations.push(savedValidation);
            } catch (error) {
              console.error(`ASYNC PROCESSING: Failed to save field ${validation.field_name}:`, error);
            }
          }

          console.log(`ASYNC PROCESSING: Successfully saved ${savedValidations.length} validations`);

          // Update session with completion status
          await storage.updateExtractionSession(sessionId, {
            status: "completed",
            extractedData: JSON.stringify(aggregatedExtraction),
            inputTokenCount: batchData?.inputTokenCount || 0,
            outputTokenCount: batchData?.outputTokenCount || 0,
            updatedAt: new Date()
          });
          console.log("ASYNC PROCESSING: Processing completed successfully");

          console.log(`ASYNC PROCESSING: Session ${sessionId} completed successfully`);
          resolve(result);

        } catch (parseError) {
          console.error('ASYNC PROCESSING: Failed to parse result:', parseError);
          
          await storage.updateExtractionSession(sessionId, {
            status: "failed",
            updatedAt: new Date()
          });
          console.error("ASYNC PROCESSING: Failed to parse AI response:", parseError.message);
    ...
    console.error("ASYNC PROCESSING: Fatal error:", error);
    ...
    try {
      ...
      console.error("ASYNC PROCESSING: Fatal error:", error.message);
    } catch (updateError) {
      console.error("ASYNC PROCESSING: Failed to update error status:", updateError);
    }
  }
}
