...
`]);
  ...
  // Background extraction endpoint - starts processing and returns immediately
  app.post("/api/sessions/:sessionId/start-background-extraction", async (req, res) => {
    try {
      const sessionId = req.params.sessionId;
      const { files, projectId } = req.body;
      
      console.log(`BACKGROUND EXTRACTION: Starting for session ${sessionId}`);
      ...
      // Return immediately to allow user to navigate
      res.json({
        success: true,
        message: "Background extraction started",
        redirect: "/dashboard" // Redirect to All Sessions view
      });
      
      // Start background processing (don't await)
      processExtractionBackground(sessionId, files, projectId).catch(error => {
        console.error(`Background extraction failed for session ${sessionId}:`, error);
        // Update session to error status
        storage.updateExtractionSession(sessionId, {
          status: 'error'
        }).catch(updateError => {
          console.error(`Failed to update session status to error:`, updateError);
        });
      });
      
    } catch (error) {
      console.error("BACKGROUND EXTRACTION setup error:", error);
      res.status(500).json({ 
        success: false, 
        error: error.message || "Failed to start background extraction" 
      });
    }
  });

  // Background processing function
  async function processExtractionBackground(sessionId: string, files: any[], projectId: string) {
    try {
      console.log(`BACKGROUND PROCESSING: Starting for session ${sessionId}`);
      
      // Step 1: Extract text from documents using existing logic
      const convertedFiles = (files || []).map((file: any) => ({
        file_name: file.name,
      ...
      console.log(`BACKGROUND STEP 1: Extracting text from ${files.length} documents`);
      ...
      const textPython = spawn('python3', ['ai_extraction_simplified.py'], {
        cwd: process.cwd()
      });
      ...
      textPython.stdin.write(JSON.stringify(textExtractionData));
      textPython.stdin.end();

      textPython.stdout.on('data', (data: any) => {
        textOutput += data.toString();
      });

      textPython.stderr.on('data', (data: any) => {
        textError += data.toString();
      });

      const textResult = await new Promise((resolve, reject) => {
        textPython.on('close', async (code: any) => {
          if (code !== 0) {
            console.error('BACKGROUND text extraction error:', textError);
            return reject(new Error(`Text extraction failed: ${textError}`));
          }
          ...
          try {
            ...
            console.error('BACKGROUND text extraction JSON parse error:', parseError);
            reject(new Error(`Invalid JSON response: ${parseError}`));
          }
        });
      });

      // Step 2: Get project data and generate AI prompt
      console.log(`BACKGROUND STEP 2: Getting project schema data`);
      const project = await storage.getProject(projectId);
      ...
      // Step 3: Generate full AI prompt using existing logic from SchemaView
      console.log(`BACKGROUND STEP 3: Generating AI prompt`);
      const documentText = textResult.extracted_texts.map((doc: any, index: number) => 
        `--- DOCUMENT ${index + 1}: ${doc.file_name} ---\n${doc.text_content}`
      ).join('\n\n--- DOCUMENT SEPARATOR ---\n\n');

      const fullPrompt = generateSchemaMarkdown(schemaData, documentText, textResult.extracted_texts.length);

      // Step 4: Process with Gemini AI
      console.log(`BACKGROUND STEP 4: Processing with Gemini AI`);
      let aiOutput = '';
      let aiError = '';
      
      const aiPython = spawn('python3', ['ai_extraction_single_step.py'], {
        cwd: process.cwd()
      });
      ...
      aiPython.stdin.write(pythonInput);
      aiPython.stdin.end();

      aiPython.stdout.on('data', (data: any) => {
        aiOutput += data.toString();
      });

      aiPython.stderr.on('data', (data: any) => {
        aiError += data.toString();
      });

      const aiResult = await new Promise((resolve, reject) => {
        aiPython.on('close', async (code: any) => {
          if (code !== 0) {
            console.error('BACKGROUND AI extraction error:', aiError);
            return reject(new Error(`AI extraction failed: ${aiError}`));
          }
          
          try {
            const result = JSON.parse(aiOutput);
            console.log('BACKGROUND AI extraction completed');
            resolve(result);
          } catch (parseError) {
            console.error('BACKGROUND AI extraction JSON parse error:', parseError);
            reject(new Error(`Invalid JSON response: ${parseError}`));
          }
        });
      });

      // Step 5: Save field validations (use existing save logic)
      console.log(`BACKGROUND STEP 5: Saving field validations`);
      if (aiResult.success && aiResult.extractedData) {
        await saveExtractionValidations(sessionId, aiResult.extractedData, schemaData);
      }

      // Step 6: Update session to in_progress status
      console.log(`BACKGROUND STEP 6: Updating session status to in_progress`);
      await storage.updateExtractionSession(sessionId, {
        status: 'in_progress'
      });

      console.log(`BACKGROUND PROCESSING: Completed successfully for session ${sessionId}`);
      
    } catch (error) {
      console.error(`BACKGROUND PROCESSING ERROR for session ${sessionId}:`, error);
      throw error; // This will trigger the error handling in the calling function
    }
  }

  // Helper function to generate schema markdown (copied from SchemaView.tsx)
  function generateSchemaMarkdown(data: any, documentText: string, documentCount: number): string {
    ...
    const schemaFieldsData = {
      schema_fields: data.schema_fields.map((field: any) => {
        const specificRules = data.extraction_rules
          .filter((rule: any) => rule.targetFields?.includes(field.fieldName))
          .map((rule: any) => rule.ruleContent);
        const globalRules = data.extraction_rules
          .filter((rule: any) => !rule.targetFields || rule.targetFields.length === 0)
          .map((rule: any) => rule.ruleContent);
    ...
    markdown += `**INSTRUCTION:** Reference these documents for validation and conflict detection. Flag any extracted values that conflict with knowledge document requirements.\n\n`;
    
    if (data.knowledge_documents.length > 0) {
      data.knowledge_documents.forEach((doc: any, index: number) => {
        const allFields = [
          ...data.schema_fields.map((field: any) => field.fieldName),
    ...
    if (data.extraction_rules.length > 0) {
      data.extraction_rules.forEach((rule: any, index: number) => {
    ...
    const outputSchema = {
      "field_validations": [
        // Schema fields
        ...data.schema_fields.map((field: any) => ({
  ...
  // Helper function to save extraction validations (based on existing save logic)
  async function saveExtractionValidations(sessionId: string, extractedData: any, schemaData: any) {
    try {
      console.log(`BACKGROUND: Saving validations for session ${sessionId}`);
      
      // Parse the extracted data JSON if it's a string
      ...
      if (typeof extractedData === 'string') {
        ...
        if (jsonMatch) {
          ...
          if (objectStart !== -1 && objectEnd !== -1) {
            const jsonText = lines.slice(objectStart, objectEnd + 1).join('\n').trim();
            parsedData = JSON.parse(jsonText);
          }
        }
      }
      
      if (!parsedData || !parsedData.field_validations) {
        throw new Error('No field_validations found in extracted data');
      }
      
      console.log(`BACKGROUND: Processing ${parsedData.field_validations.length} field validations`);
      
      // Save each field validation
      for (const validation of parsedData.field_validations) {
        const validationData = {
          sessionId: sessionId,
          ...
          documentSections: validation.document_sections ? JSON.stringify(validation.document_sections) : null
        };
        
        await storage.createFieldValidation(validationData);
      }
      
      console.log(`BACKGROUND: Successfully saved ${parsedData.field_validations.length} validations`);
      
    } catch (error) {
      console.error(`BACKGROUND: Error saving validations for session ${sessionId}:`, error);
      throw error;
    }
  }

  // Gemini AI extraction endpoint (keep existing for manual processing)
