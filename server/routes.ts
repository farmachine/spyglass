...
`], {
  ...
  // RETRY: Retry failed extraction for sessions stuck at "text_extracted"
  app.post("/api/sessions/:sessionId/retry-extraction", async (req, res) => {
    try {
      const sessionId = req.params.sessionId;
      const session = await storage.getExtractionSession(sessionId);
      ...
      if (session.status !== "text_extracted") {
        return res.status(400).json({ 
          message: `Session status is ${session.status}, can only retry sessions with status "text_extracted"` 
        });
      }
      
      console.log(`RETRY: Retrying extraction for failed session ${sessionId}`);
      
      // Get project data for the retry
      ...
      // Since we don't have the original files, we need to simulate with empty files
      // This will force the AI to work with whatever text was previously extracted
      const extractionData = {
        step: "extract",
        session_id: sessionId,
        files: [], // Empty since text was already extracted
        project_schema: {
        ...
        extraction_rules: extractionRules,
        knowledge_documents: knowledgeDocuments,
        session_name: project.mainObjectName || "document"
      };
      
      console.log(`RETRY: Re-extracting with ${schemaFields.length} schema fields and ${collectionsWithProperties.length} collections`);
      ...
      python.on('close', async (code: any) => {
        if (code !== 0) {
          ...
          return res.status(500).json({ 
            message: "Retry extraction failed", 
            error: error 
          });
        }
        ...
        try {
          const extractedData = JSON.parse(output);
          console.log('RETRY: Successfully extracted data, validations count:', extractedData.field_validations?.length || 0);
          
          // Clear existing placeholder validations first
          ...
          res.json({ 
            ...
            extractedData,
            sessionId,
            validationCount: extractedData.field_validations?.length || 0
          });
          
        } catch (parseError) {
          console.error('RETRY processing error:', parseError);
          res.status(500).json({ message: "Failed to process retry extraction results" });
        }
      });
      
    } catch (error) {
      console.error("RETRY extraction error:", error);
      res.status(500).json({ message: "Failed to start retry process" });
    }
  });
