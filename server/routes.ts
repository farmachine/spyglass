...
import { 
  ...
  insertStepSchemaFieldSchema,
  insertStepCollectionSchema,
  insertStepCollectionPropertySchema,
  insertStepReferenceSchema
} from "@shared/schema";
...
`]);
  ...
  // Gemini AI extraction endpoint for schema processing
  app.post("/api/sessions/:sessionId/gemini-extraction", async (req, res) => {
    try {
      const sessionId = req.params.sessionId;
...
`]);
  ...
  // Extraction Steps Routes

  // Get all extraction steps for a project
  app.get("/api/projects/:projectId/extraction-steps", async (req, res) => {
    try {
      const projectId = req.params.projectId;
  ...
  // Get a specific extraction step
  app.get("/api/extraction-steps/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const step = await storage.getExtractionStep(id);
      if (!step) {
        return res.status(404).json({ message: "Extraction step not found" });
      }
      res.json(step);
    } catch (error) {
      console.error("Error getting extraction step:", error);
      res.status(500).json({ message: "Failed to get extraction step" });
    }
  });

  // Create a new extraction step
  app.post("/api/projects/:projectId/extraction-steps", async (req, res) => {
    try {
      const projectId = req.params.projectId;
      const result = insertExtractionStepSchema.safeParse({
        ...req.body,
        projectId
      });
  ...
  // Update an extraction step
  app.put("/api/extraction-steps/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const result = insertExtractionStepSchema.partial().safeParse(req.body);
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid extraction step data", errors: result.error.errors });
      }

      const step = await storage.updateExtractionStep(id, result.data);
      if (!step) {
        return res.status(404).json({ message: "Extraction step not found" });
      }
  ...
  // Delete an extraction step
  app.delete("/api/extraction-steps/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const success = await storage.deleteExtractionStep(id);
      
      if (success) {
        res.json({ message: "Extraction step deleted successfully" });
      } else {
        res.status(404).json({ message: "Extraction step not found" });
      }
    } catch (error) {
      console.error("Error deleting extraction step:", error);
      res.status(500).json({ message: "Failed to delete extraction step" });
    }
  });

  // Step Schema Fields Routes

  // Create a step schema field
  app.post("/api/extraction-steps/:stepId/schema-fields", async (req, res) => {
    try {
      const stepId = req.params.stepId;
      const result = insertStepSchemaFieldSchema.safeParse({
        ...req.body,
        extractionStepId: stepId
      });
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step schema field data", errors: result.error.errors });
      }

      const field = await storage.createStepSchemaField(result.data);
      res.status(201).json(field);
    } catch (error) {
      console.error("Error creating step schema field:", error);
      res.status(500).json({ message: "Failed to create step schema field" });
    }
  });

  // Update a step schema field
  app.put("/api/step-schema-fields/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const result = insertStepSchemaFieldSchema.partial().safeParse(req.body);
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step schema field data", errors: result.error.errors });
      }

      const field = await storage.updateStepSchemaField(id, result.data);
      if (!field) {
        return res.status(404).json({ message: "Step schema field not found" });
      }
      
      res.json(field);
    } catch (error) {
      console.error("Error updating step schema field:", error);
      res.status(500).json({ message: "Failed to update step schema field" });
    }
  });

  // Delete a step schema field
  app.delete("/api/step-schema-fields/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const success = await storage.deleteStepSchemaField(id);
      
      if (success) {
        res.json({ message: "Step schema field deleted successfully" });
      } else {
        res.status(404).json({ message: "Step schema field not found" });
      }
    } catch (error) {
      console.error("Error deleting step schema field:", error);
      res.status(500).json({ message: "Failed to delete step schema field" });
    }
  });

  // Step Collections Routes

  // Create a step collection
  app.post("/api/extraction-steps/:stepId/collections", async (req, res) => {
    try {
      const stepId = req.params.stepId;
      const result = insertStepCollectionSchema.safeParse({
        ...req.body,
        extractionStepId: stepId
      });
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step collection data", errors: result.error.errors });
      }

      const collection = await storage.createStepCollection(result.data);
      res.status(201).json(collection);
    } catch (error) {
      console.error("Error creating step collection:", error);
      res.status(500).json({ message: "Failed to create step collection" });
    }
  });

  // Update a step collection
  app.put("/api/step-collections/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const result = insertStepCollectionSchema.partial().safeParse(req.body);
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step collection data", errors: result.error.errors });
      }

      const collection = await storage.updateStepCollection(id, result.data);
      if (!collection) {
        return res.status(404).json({ message: "Step collection not found" });
      }
      
      res.json(collection);
    } catch (error) {
      console.error("Error updating step collection:", error);
      res.status(500).json({ message: "Failed to update step collection" });
    }
  });

  // Delete a step collection
  app.delete("/api/step-collections/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const success = await storage.deleteStepCollection(id);
      
      if (success) {
        res.json({ message: "Step collection deleted successfully" });
      } else {
        res.status(404).json({ message: "Step collection not found" });
      }
    } catch (error) {
      console.error("Error deleting step collection:", error);
      res.status(500).json({ message: "Failed to delete step collection" });
    }
  });

  // Step Collection Properties Routes

  // Create a step collection property
  app.post("/api/step-collections/:collectionId/properties", async (req, res) => {
    try {
      const collectionId = req.params.collectionId;
      const result = insertStepCollectionPropertySchema.safeParse({
        ...req.body,
        stepCollectionId: collectionId
      });
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step collection property data", errors: result.error.errors });
      }

      const property = await storage.createStepCollectionProperty(result.data);
      res.status(201).json(property);
    } catch (error) {
      console.error("Error creating step collection property:", error);
      res.status(500).json({ message: "Failed to create step collection property" });
    }
  });

  // Update a step collection property
  app.put("/api/step-collection-properties/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const result = insertStepCollectionPropertySchema.partial().safeParse(req.body);
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step collection property data", errors: result.error.errors });
      }

      const property = await storage.updateStepCollectionProperty(id, result.data);
      if (!property) {
        return res.status(404).json({ message: "Step collection property not found" });
      }
      
      res.json(property);
    } catch (error) {
      console.error("Error updating step collection property:", error);
      res.status(500).json({ message: "Failed to update step collection property" });
    }
  });

  // Delete a step collection property
  app.delete("/api/step-collection-properties/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const success = await storage.deleteStepCollectionProperty(id);
      
      if (success) {
        res.json({ message: "Step collection property deleted successfully" });
      } else {
        res.status(404).json({ message: "Step collection property not found" });
      }
    } catch (error) {
      console.error("Error deleting step collection property:", error);
      res.status(500).json({ message: "Failed to delete step collection property" });
    }
  });

  // Step References Routes

  // Create a step reference
  app.post("/api/extraction-steps/:stepId/references", async (req, res) => {
    try {
      const stepId = req.params.stepId;
      const result = insertStepReferenceSchema.safeParse({
        ...req.body,
        toStepId: stepId
      });
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step reference data", errors: result.error.errors });
      }

      const reference = await storage.createStepReference(result.data);
      res.status(201).json(reference);
    } catch (error) {
      console.error("Error creating step reference:", error);
      res.status(500).json({ message: "Failed to create step reference" });
    }
  });

  // Update a step reference
  app.put("/api/step-references/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const result = insertStepReferenceSchema.partial().safeParse(req.body);
      
      if (!result.success) {
        return res.status(400).json({ message: "Invalid step reference data", errors: result.error.errors });
      }

      const reference = await storage.updateStepReference(id, result.data);
      if (!reference) {
        return res.status(404).json({ message: "Step reference not found" });
      }
      
      res.json(reference);
    } catch (error) {
      console.error("Error updating step reference:", error);
      res.status(500).json({ message: "Failed to update step reference" });
    }
  });

  // Delete a step reference
  app.delete("/api/step-references/:id", async (req, res) => {
    try {
      const id = req.params.id;
      const success = await storage.deleteStepReference(id);
      
      if (success) {
        res.json({ message: "Step reference deleted successfully" });
      } else {
        res.status(404).json({ message: "Step reference not found" });
      }
    } catch (error) {
      console.error("Error deleting step reference:", error);
      res.status(500).json({ message: "Failed to delete step reference" });
    }
  });
