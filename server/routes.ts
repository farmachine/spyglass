...
`]);
  ...
  // AUTOMATED EXTRACTION ENDPOINT - Replicates debug workflow exactly without user interaction
  app.post("/api/sessions/:sessionId/automated-extraction", async (req, res) => {
    try {
      const sessionId = req.params.sessionId;
      const { debugMode } = req.body;
      
      console.log(`AUTOMATED EXTRACTION: Starting automated workflow for session ${sessionId}`);
      ...
      // Get extracted text from session (should already be there from extract-text call)
      ...
      console.log(`AUTOMATED EXTRACTION: Found ${extractedTextData.extracted_texts.length} extracted documents`);
      
      // STEP 1: Call the exact same extraction logic that debug workflow uses
      ...
      const extractionRequestData = {
        files: extractedTextData.extracted_texts.map((doc: any) => ({
          name: doc.file_name,
          content: `data:text/plain;base64,${Buffer.from(doc.text_content).toString('base64')}`,
          type: 'text/plain'
        })),
        project_data: {
      ...
      console.log(`AUTOMATED EXTRACTION: Calling STEP 1 AI extraction with ${extractionRequestData.files.length} files`);
      
      // Use exact same Python extraction call that debug workflow uses
      const { spawn } = await import('child_process');
      const python = spawn('python3', ['ai_extraction_single_step.py'], {
        cwd: process.cwd()
      });
      ...
      const extractionResult = await new Promise((resolve, reject) => {
        python.on('close', async (code: any) => {
          if (code !== 0) {
            console.error('AUTOMATED EXTRACTION STEP 1 error:', error);
            return reject(new Error(`AI extraction failed: ${error}`));
          }
          
          try {
            const result = JSON.parse(output);
            console.log('AUTOMATED EXTRACTION: STEP 1 AI extraction completed successfully');
            
            // Update session with extraction results (same as debug workflow)
            await storage.updateExtractionSession(sessionId, {
              status: "extracted",
              extractedData: JSON.stringify(result)
            });
            ...
            console.error('AUTOMATED EXTRACTION STEP 1 JSON parse error:', parseError);
            console.error('Raw output:', output);
            reject(new Error(`Invalid JSON response: ${parseError}`));
          }
        });
      ...
      console.log(`AUTOMATED EXTRACTION: STEP 2 creating field validation records`);
      
      // STEP 2: Create field validation records (same as debug workflow)
      if (extractionResult && extractionResult.extractedData) {
        await createFieldValidationRecords(sessionId, extractionResult.extractedData, {
          projectId: session.projectId,
          schemaFields: schemaFields,
          collections: collections,
          mainObjectName: project.mainObjectName
        });
        console.log(`AUTOMATED EXTRACTION: STEP 2 field validations created successfully`);
      }
      
      console.log(`AUTOMATED EXTRACTION: STEP 3 running batch validation`);
      
      // STEP 3: Run batch validation (same as debug workflow)
      const validations = await storage.getSessionValidations(sessionId);
      if (validations.length > 0) {
        ...
        // Call Python batch validation
        const pythonValidation = spawn('python3', ['ai_extraction_single_step.py'], {
          cwd: process.cwd()
        });
        
        pythonValidation.stdin.write(JSON.stringify({
          action: "validate_batch",
          ...validationData
        }));
        pythonValidation.stdin.end();
        
        let validationOutput = '';
        let validationError = '';
        
        pythonValidation.stdout.on('data', (data: any) => {
          validationOutput += data.toString();
        });
        
        pythonValidation.stderr.on('data', (data: any) => {
          validationError += data.toString();
        });
        
        await new Promise((resolve, reject) => {
          pythonValidation.on('close', async (code: any) => {
            if (code !== 0) {
              console.error('AUTOMATED EXTRACTION STEP 3 validation error:', validationError);
              return resolve(null); // Don't fail, just skip validation
            }
            
            try {
              const validationResult = JSON.parse(validationOutput);
              
              if (validationResult.updated_validations) {
                for (const validation of validationResult.updated_validations) {
                  await storage.updateFieldValidation(validation.id, {
                    confidenceScore: validation.confidence_score,
                    aiReasoning: validation.ai_reasoning,
                    validationStatus: validation.confidence_score >= 80 ? "verified" : "unverified"
                  });
                }
                console.log(`AUTOMATED EXTRACTION: STEP 3 batch validation completed for ${validationResult.updated_validations.length} fields`);
              }
              
              resolve(validationResult);
              
            } catch (parseError) {
              console.error('AUTOMATED EXTRACTION STEP 3 JSON parse error:', parseError);
              resolve(null); // Don't fail, just skip validation
            }
          });
        });
      }
      
      console.log(`AUTOMATED EXTRACTION: All steps completed successfully`);
