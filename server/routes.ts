...
`], {
  ...
  // Extraction Wizard - Run extraction from wizard modal
  app.post("/api/run-extraction-wizard", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const { selectedDocuments, selectedFields, extractionRules, additionalInstructions } = req.body;
      
      console.log('EXTRACTION WIZARD: Starting wizard extraction');
      console.log(`EXTRACTION WIZARD: Documents: ${selectedDocuments?.length || 0}`);
      console.log(`EXTRACTION WIZARD: Fields: ${selectedFields?.length || 0}`);
      console.log(`EXTRACTION WIZARD: Rules: ${extractionRules?.total || 0}`);
      
      // Prepare data for Python extraction wizardry script
      const wizardData = {
        selectedDocuments: selectedDocuments || [],
        selectedFields: selectedFields || [],
        extractionRules: extractionRules || { targeted: [], global: [], total: 0 },
        additionalInstructions: additionalInstructions || ""
      };
      
      // Call Python extraction wizardry script
      const python = spawn('python3', ['extraction_wizardry.py']);
      
      python.stdin.write(JSON.stringify(wizardData));
      python.stdin.end();
      ...
      python.on('close', (code: any) => {
        if (code !== 0) {
          console.error('EXTRACTION WIZARD error:', error);
          return res.status(500).json({ 
            message: "Extraction wizard failed", 
            error: error 
          });
        }
        ...
        try {
          ...
          const jsonResponseIndex = lines.findIndex(line => line.includes('JSON RESPONSE:'));
          
          if (jsonResponseIndex !== -1 && jsonResponseIndex < lines.length - 1) {
            const jsonLines = lines.slice(jsonResponseIndex + 1);
            const jsonResponse = jsonLines.join('\n');
            const result = JSON.parse(jsonResponse);
            
            console.log('EXTRACTION WIZARD: Success');
            res.json(result);
          } else {
          ...
          console.error('EXTRACTION WIZARD parse error:', parseError);
          console.error('EXTRACTION WIZARD raw output:', output);
          res.status(500).json({ 
            message: "Failed to parse extraction wizard response",
            error: parseError,
            output: output
          });
        }
      ...
      console.error("EXTRACTION WIZARD error:", error);
      res.status(500).json({ message: "Failed to run extraction wizard" });
    }
  });
