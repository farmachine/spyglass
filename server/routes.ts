...
import { 
  ...
  insertTestDocumentSchema,
  insertTestDataSetSchema
} from "@shared/schema";
...
`]);
  ...
  app.post("/api/projects/:projectId/test-documents", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const { name, fileName, fileSize, mimeType, file } = req.body;
      const fileBuffer = Buffer.from(file, 'base64');
      
      // Determine document type from file extension or MIME type
      let documentType: 'Excel' | 'Word' | 'PDF' = 'PDF';
      ...
      try {
        const pythonProcess = spawn('python3', [
          documentType === 'Excel' ? 'enhanced_excel_extractor.py' : 'document_extractor.py'
        ], {
          stdio: ['pipe', 'pipe', 'pipe']
        });
        ...
        extractedContent = output.trim();
      } catch (error) {
        console.error("Document processing failed:", error);
        extractedContent = "Error processing document content";
      }

      const result = insertTestDocumentSchema.safeParse({
        projectId,
        name,
        fileName,
        fileSize,
      ...
      if (!result.success) {
        return res.status(400).json({ message: "Invalid test document data", errors: result.error.errors });
      }

      const document = await storage.createTestDocument(result.data);
      res.status(201).json(document);
    } catch (error) {
  ...
  // Test Data Sets Routes
  
  // Get test data sets for a project
  app.get("/api/projects/:projectId/test-data-sets", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const { projectId } = req.params;
      const dataSets = await storage.getTestDataSets(projectId);
      res.json(dataSets);
    } catch (error) {
      console.error("Error getting test data sets:", error);
      res.status(500).json({ message: "Failed to get test data sets" });
    }
  });

  // Create test data set
  app.post("/api/projects/:projectId/test-data-sets", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const { projectId } = req.params;
      const { name, description, jsonData } = req.body;
      const userId = req.user?.id;
      ...
      const result = insertTestDataSetSchema.safeParse({
        projectId,
        name,
        description,
        jsonData,
      ...
      if (!result.success) {
        return res.status(400).json({ message: "Invalid test data set data", errors: result.error.errors });
      }

      const dataSet = await storage.createTestDataSet(result.data);
      res.status(201).json(dataSet);
    } catch (error) {
      console.error("Error creating test data set:", error);
      res.status(500).json({ message: "Failed to create test data set" });
    }
  });

  // Delete test data set
  app.delete("/api/test-data-sets/:id", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const deleted = await storage.deleteTestDataSet(id);
      
      if (!deleted) {
        return res.status(404).json({ message: "Test data set not found" });
      }
      
      res.status(204).send();
    } catch (error) {
      console.error("Error deleting test data set:", error);
      res.status(500).json({ message: "Failed to delete test data set" });
    }
  });
