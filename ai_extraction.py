...
def step1_extract_from_documents(
    ...
    """
    STEP 1: Extract data from documents using AI (consolidated from simplified version)
    ...
    Args:
        ...
        extraction_rules: Rules for extraction modification
    ...
    try:
        ...
        api_key = os.environ.get("GEMINI_API_KEY")
        if not api_key:
            return {"success": False, "error_message": "GEMINI_API_KEY not found"}
        ...
        # Use the comprehensive extraction from the first document approach
        if documents:
            first_doc = documents[0]
            extraction_result = extract_data_from_document(
                file_content=first_doc['file_content'],
                file_name=first_doc['file_name'],
                mime_type=first_doc['mime_type'],
            ...
            return {"success": False, "error_message": "No documents provided"}
...
def step2_validate_field_records(
    ...
    """
    STEP 2: Validate field records using AI analysis (consolidated from simplified version)
    
    Args:
        field_validations: List of field validation records to validate
        extraction_rules: Rules for confidence adjustment
        knowledge_documents: Knowledge documents for conflict detection
    
    Returns:
        ValidationResult with updated validations
    """
    try:
        # Check for API key
        api_key = os.environ.get("GEMINI_API_KEY")
        ...
        # Use the existing ai_validate_batch function
        updated_validations = []
        
        for fv in field_validations:
            field_name = fv.get('fieldName', '')
            extracted_value = fv.get('extractedValue', '')
            
            # Apply rules-based confidence calculation
            confidence = calculate_knowledge_based_confidence(
                field_name, extracted_value, extraction_rules or [], knowledge_documents or []
            )
            
            # Update the validation record
            updated_fv = fv.copy()
            updated_fv['confidenceScore'] = confidence
            updated_fv['validationStatus'] = 'verified' if confidence >= 80 else 'unverified'
            updated_validations.append(updated_fv)
        
        logging.info(f"STEP 2: Successfully validated {len(updated_validations)} field records")
        return {"success": True, "fieldValidations": updated_validations}
...
# Main function for command-line execution (consolidated from simplified version)
    ...
    try:
        ...
        if step == "extract":
            ...
            if result.get("success"):
                ...
                print(json.dumps({"error": result.get("error_message", "Unknown error")}), file=sys.stderr)
                sys.exit(1)
            ...
            if result.get("success"):
                print(json.dumps(result.get("fieldValidations", [])))
            else:
                print(json.dumps({"error": result.get("error_message", "Unknown error")}), file=sys.stderr)
                sys.exit(1)
