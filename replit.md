...
- **Sequential Extraction Engine**: Advanced field-by-field extraction system (`sequential_extraction_engine.py`) that processes fields in order: identifier fields first, then subsequent fields using identifier references as input. Each extraction step creates validation results for all identified records. For a project with 185 identifiers and 2 target properties, this creates 370 validations (2 per identifier). The system automatically determines extraction methods (Excel wizardry vs AI) per field and ensures null validations are created when values cannot be found for specific identifiers. Supports both the existing simultaneous extraction and the new sequential approach.
