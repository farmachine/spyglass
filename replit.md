...
- **Extraction Step Parameters Database Persistence**: Added comprehensive database persistence for extraction step parameters to handle timeout issues and allow process resumption. Created extraction_step_parameters table with session_id, project_id, extraction_number, target_property details, extraction method, function_id, parameters (JSONB), status tracking, and timestamps. Enhanced both MemStorage and DatabaseStorage classes with full CRUD operations, API routes for step management, and integrated database saving/updating throughout the extraction_wizardry.py workflow for reliable state persistence across extraction steps.
