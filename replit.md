...
- **Enhanced Extraction Processor**: Complete rewrite of extraction system (`enhanced_extraction_processor.py`) that intelligently routes based on field configuration. FUNCTION extraction type routes directly to Excel functions by ID, while AI extraction type generates dynamic prompts using field metadata including knowledge documents, extraction rules, and previous extractions. This eliminates manual prompt crafting and leverages the rich field configuration data for context-aware extraction.
- **Dynamic AI Prompt Generation**: AI extractions now automatically build prompts using the field's knowledge document IDs, extraction rule IDs, descriptions, and previously verified data for context. Each field gets a customized prompt based on its specific configuration, dramatically improving extraction accuracy and consistency.
- **Intelligent Extraction Routing**: The system now respects the extraction_type field configuration, sending FUNCTION fields directly to their assigned Excel functions and AI fields to context-aware AI processing, resulting in more efficient and accurate extractions.
