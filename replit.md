...
- **Fixed Excel Wizardry Function Cross-Session Persistence**: Resolved function ID parsing issues with extra characters (asterisks, quotes) and updated all 23 existing functions to use proper field schema mapping (field_id from target field 'id', collection_id from 'collectionId'). Functions now persist correctly across all sessions within a project for efficient reuse of extraction logic.
