...
- **Unified Function-Based Property Configuration**: Completely removed duplicate AI extraction type UI from CollectionCard component that was causing duplicate "AI Instructions" fields. Enhanced @-key reference system to include all properties from previous collections when collection index matching fails (index = -1), ensuring "Data Field" and "Worksheet" references are always available for parameter autocomplete. Simplified CollectionCard to show only basic property description, with all extraction configuration now handled through the unified PropertyDialog function-based interface.
...
- **Streamlined Field Configuration UI**: Updated field configuration modals (both SchemaFieldDialog and PropertyDialog) to match admin area styling with dark grey/light grey theme. Simplified modal structure to focus purely on function parameters: Step 1 (Function & Data Sources) contains function selection and parameter configuration inputs only; Step 2 (Field Settings) contains basic field configuration including dynamic choices management for "Choice" data type fields. Document parameters use dropdown selectors with "User Provided Document" option plus any uploaded knowledge documents. Users can add/remove individual choice options with intuitive UI controls. This provides a much simpler and more intuitive configuration experience for field extraction setup.
- **Multi-Select Document References**: Enhanced PropertyDialog to support multi-select document parameters instead of single-select. Created custom MultiSelectDocument component with dropdown functionality, click-outside handling, badge display for selected documents, and support for both "User Provided Document" and knowledge documents. Users can now select multiple reference documents for extraction parameters with intuitive UI controls.
- **Comprehensive Excel Function Logging**: Added detailed console logging for all Excel function operations including creation (POST), updates (PUT/PATCH), retrieval (GET), deletion (DELETE), and usage increments. Logs include clear markers (=== FUNCTION CREATED/UPDATED ===), full function objects on success, validation errors, and operation status. Console logging successfully outputs complete function objects, though large JSON responses may be truncated in console display while maintaining start/end markers for verification. This provides complete visibility into function lifecycle management for debugging and monitoring purposes.
