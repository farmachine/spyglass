...
- **PostgreSQL Job Queue System**: Complete asynchronous job processing system with extraction_jobs, job_dependencies, and extraction_cache tables. Prevents server timeout errors during long-running extractions by processing jobs in background with real-time progress tracking. Maintains sequential extraction workflow (0→1→2...) while providing user-friendly job monitoring through JobProgress React component. Jobs execute Python extraction scripts asynchronously with proper error handling, timeout management, and completion callbacks that refresh UI data automatically.
