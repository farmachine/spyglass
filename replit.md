...
- **AI Model Configuration at Tool Level Only** (January 2025): Removed AI model selection from workflow builder interface. AI models are now configured only at tool creation time and stored in the database. The tool's configured model is used during execution, not workflow input values.
- **Fixed Excel Content Extraction for Tools** (January 2025): Corrected field names in toolEngine.ts for document extraction (file_name, mime_type, file_content) to match document_extractor.py expectations, resolving "No column headers found" errors in Excel processing functions.
