...
- **Navigation**: Consolidated Project Configuration interface (renamed to "Configure" January 2025) with URL path `/projects/:id/configure` and streamlined session view navigation with integrated left sidebar. Removed redundant "Schema Information" section from Project Admin sidebar (January 2025). Clear distinction: "extrapl • Configure" for project configuration pages, "extrapl • Admin" for system administration (users/organizations).
...
- **Fixed Knowledge Document Loading** (January 2025): Resolved issue where knowledge documents weren't being loaded in AI tools. Fixed incorrect field reference (`doc.title` → `doc.displayName`) in toolEngine.ts. Knowledge documents now properly load content for AI processing in workflow steps.
- **Enhanced Field Validation Mapping** (January 2025): Implemented proper identifierId-based mapping for field validations. System now correctly uses identifierId from tool results to map to existing validation records, enabling proper update of records rather than creating duplicates.
...
- **Workflow Test Document Handling** (January 2025): Fixed document parameter replacement in workflow test execution. System now properly detects ["user_document"] placeholder and replaces with actual test document content. Enhanced AI tool array processing to explicitly require processing ALL input items with result count validation and automatic padding.
- **AI Tool Array Processing Fix** (January 2025): Fixed issue where AI tools returned only 5 results for large input arrays. Implemented enhanced batching strategy specifically for AI tools with smaller batch size (5 items) and lower threshold to ensure complete processing of all inputs. AI tools now process arrays in manageable chunks with validation to ensure correct result count.
...
- **Unified Database Architecture** (January 2025): Restructured database to treat schema fields and collections consistently as "steps" with "values". New `workflow_steps` table stores both Info Pages (single values/schema) and Data Tables (multiple records/collections). Each step tracks `valueCount` (number of values) and `identifierId` (UUID of first value for list steps). New `step_values` table replaces both schema fields and collection properties, storing value configuration including `toolId` and `inputValues` for tool parameters and @references. Legacy tables maintained for backward compatibility during migration. **Important**: In field_validations table, `collectionId` now references the stepId (not old collection ID), maintaining parent-child relationship where Steps are containers and Values are the columns/fields within them.
- **Session View Workflow Architecture** (January 2025): Transitioning SessionView to work with unified step/value architecture. Field validations now include `stepId`, `valueId`, and `identifierId` columns to support the new workflow system. Session extraction will process steps sequentially, storing results after each function completes, enabling proper reference resolution for subsequent steps. Sidebar displays steps, main content shows values (fields for Info Pages, columns for Data Tables).
