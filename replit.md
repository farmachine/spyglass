...
- **AI Tool Array Processing Issue** (January 2025): Identified issue where AI tools return only 5 results regardless of input array size (e.g., 5 results for 185 inputs). The "AI Query Against Document" tool has correct prompt instructions requiring exact count matching, but Gemini AI model is not following the instruction. Requires prompt enhancement or batching strategy adjustment.
