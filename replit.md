...
- **Modal-Based AI Extraction**: Implemented targeted extraction functionality with `/api/sessions/:sessionId/modal-extraction` endpoint. Users can select specific documents, reference data, and target fields through horizontal button interface. Latest update: Fixed critical field ID mapping issue - backend was using constructed IDs like `collection.property` while frontend sends actual UUID field IDs. Updated backend to use proper UUID-based field matching for both schema fields and collection properties. Ready for testing with corrected field filtering logic.
