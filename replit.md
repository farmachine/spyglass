...
- **Enhanced Field Configuration UI**: Updated field configuration modals (both SchemaFieldDialog and PropertyDialog) to match admin area styling with dark grey/light grey theme. Replaced @ symbol references with cleaner terminology: "Reference Data" → "source data" and "Reference Documents" → "source documents". Transformed complex document selection dropdowns into simple checkbox-based user document upload requirements, while maintaining separate knowledge document selections that exclude user-uploaded files. Consolidated modal structure from 4 steps to 2 streamlined steps: Step 1 (Function & Data Sources) combines function selection, parameter configuration, and source documents/rules; Step 2 (Field Settings) contains basic field configuration. This provides a cleaner, more intuitive configuration experience for field extraction setup.
