...
- **Streamlined Field Configuration UI**: Updated field configuration modals (both SchemaFieldDialog and PropertyDialog) to match admin area styling with dark grey/light grey theme. Simplified modal structure to focus purely on function parameters: Step 1 (Function & Data Sources) contains function selection and parameter configuration inputs only; Step 2 (Field Settings) contains basic field configuration. Removed redundant document and rule selection fields, making the interface cleaner and more focused on the essential function parameter inputs. This provides a much simpler and more intuitive configuration experience for field extraction setup.
