...
- **Modal-Based AI Extraction**: Implemented targeted extraction functionality with `/api/sessions/:sessionId/modal-extraction` endpoint. Users can select specific documents, reference data, and target fields through horizontal button interface. Current debugging status: Fixed data structure mismatches (`filename` → `file_name`, `content` → `file_content`) but experiencing issues with empty document content and field selection filtering. Enhanced with comprehensive debugging logs to track document content lengths and field mapping.
